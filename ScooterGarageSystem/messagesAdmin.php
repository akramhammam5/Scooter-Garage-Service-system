...
if(isset($_POST['search'])) {
	$search=$_POST['name'];
	$searchUser = "SELECT * FROM learnersignup WHERE Username = '$search'";
	...
	while($searchUserRow = mysqli_fetch_array($searchUserResult)){	?>
		...
		<a href="./messageAdmin.php?receiver=<?=$searchUserRow['LearnerID']?>">Send message</a>
		</div>
<?php }
}
?>
<div>
<?php
$lastMessage = "SELECT DISTINCT SenderID FROM messages WHERE ReceiverID = ".$_SESSION['ID'];
...
if(mysqli_num_rows($lastMessageResult) > 0) {
	while($lastMessageRow = mysqli_fetch_array($lastMessageResult)) {
		...
		$getSender = "SELECT * FROM adminregistration WHERE ID = '$sent_by'";
